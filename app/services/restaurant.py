...
class RestaurantService:
        ...
        *,
        ...
        if fulfillment_at:
            if fulfillment not in {"pickup", "delivery"}:
                raise RestaurantServiceError("Only pickup or delivery accepts a requested time.", code="invalid_fulfillment_time")
            now = _restaurant_now()
            try:
                requested_time = datetime.fromisoformat(fulfillment_at)
            except (TypeError, ValueError) as exc:
                raise RestaurantServiceError("Use an ISO date and time with timezone.", code="invalid_fulfillment_time") from exc
            if requested_time.tzinfo is None:
                raise RestaurantServiceError("The requested time needs a timezone.", code="invalid_fulfillment_time")
            requested_time = requested_time.astimezone(now.tzinfo)
            if requested_time < now + timedelta(minutes=_fulfillment_delay_minutes(fulfillment)):
                raise RestaurantServiceError("That time is too soon for preparation. Please choose a later time.", code="fulfillment_time_too_soon", status=409)
            schedule = get_restaurant_knowledge().schedule_status(fulfillment, requested_time)
            if not schedule.get("available"):
                raise RestaurantServiceError(str(schedule.get("customer_message") or "That fulfillment time is unavailable."), code="fulfillment_time_unavailable", status=409)
            payload["fulfillment_at"] = requested_time.isoformat()
        ...
        async def operation(conn: Any) -> JsonDict:
            ...
            if fulfillment in {"pickup", "delivery"}:
                service_at = _fulfillment_at(
                    fulfillment,
                    {"fulfillment_at": requested_time.isoformat()} if requested_time else {},
                    now=_restaurant_now(),
                )
                fulfillment_details["fulfillment_at"] = service_at.isoformat()
            await self._ensure_order_items_valid_at(
    ...
    async def abandon_pending_order(
        self, *, call_id: str, idempotency_key: str
    ) -> JsonDict:
        """Abandon only this session's unconfirmed draft; keep its booking."""
        call_id = self._require_call_id(call_id)
        payload = {"call_id": call_id, "action": "abandon_pending_order"}
        ...
        async def operation(conn: Any) -> JsonDict:
            ...
            if order is None:
                raise RestaurantServiceError(
                    "There is no unconfirmed pre-order to leave behind.",
            ...
            summary = await self._order_summary_with_conn(conn, order["id"])
            return {**summary, "abandoned": True}

        result, replayed = await self._idempotent_write(
            action="abandon_pending_order",
            idempotency_key=idempotency_key,
            call_id=call_id,
            payload=payload,
            operation=operation,
