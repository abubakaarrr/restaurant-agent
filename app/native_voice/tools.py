...
class RestaurantToolExecutor:
    ...
    async def _invoke_native(self, name: str, arguments: Mapping[str, Any]) -> Any:
        ...
        if name == "check_table_availability":
            # A caller changing their own confirmed reservation must not make
            # their existing table appear occupied by another customer.
            ...
            if self._native_service is not None:
                ...
                if str(draft.get("status") or "") == "confirmed":
                    exclude_booking_id = int(draft.get("booking_id") or 0)
        ...
        if name == "get_full_menu":
            return await self._service.list_menu(available_only=False, **({"at": self.menu_at} if self.menu_at else {}))
        if name == "check_menu_item_availability":
            return await self._service.find_menu_item(str(args.get("item_name") or ""), **({"at": self.menu_at} if self.menu_at else {}))
        ...
        if name == "set_order_fulfillment":
            return await self._service.set_order_fulfillment(
                ...
                **({"fulfillment_at": str(args["fulfillment_at"])} if args.get("fulfillment_at") else {}),
            )
        ...
        if name == "abandon_pending_order":
            return await self._service.abandon_pending_order(
                call_id=session_id,
                idempotency_key=key,
            )
    ...
    async def invoke(self, name: str, arguments: Mapping[str, Any]) -> Any:
        ...
        if name == "get_full_menu":
            result = await self._service.list_menu(available_only=False, **({"at": self.menu_at} if self.menu_at else {}))
            return {
                **result,
        ...
        if name == "check_menu_item_availability":
            result = await self._service.find_menu_item(str(arguments.get("item_name") or ""), **({"at": self.menu_at} if self.menu_at else {}))
            return {
                **result,
    ...
    async def readback(self, name: str, arguments: Mapping[str, Any], result: Any) -> Any:
        ...
        if name == "abandon_pending_order" and isinstance(result, Mapping):
    ...
    definitions = [
        ...
        _tool_definition(
            ...
            "Set dine-in, pickup, or synthetic local delivery details.",
            {
                ...
                "fulfillment_at": {"type": "string", "description": "Requested pickup/delivery ISO datetime with timezone offset. Omit only if the caller did not request a time."},
            },
    ...
    async def invoke(
        ...
        *,
        ...
        try:
            ...
            # Preserve application error codes, never exception messages or caller
            # data. The controller needs to distinguish missing approval, a full
            # slot, and a connection failure to recover correctly.
            if isinstance(exc, RestaurantServiceError) and re.fullmatch(r"[a-z_]{1,64}", str(exc.code)):
                error = exc.code
            outcome = ToolOutcome(name=name, call_id=call_id, arguments=args, result=None, success=False, error=error, state_version=state_version)
            self._calls[call_id] = outcome
            return outcome
        finally:
        ...
        if isinstance(readback, Mapping):
            ...
            if "order_id" in readback:
                ...
                if not isinstance(readback.get("readback_hash"), str) or readback["readback_hash"] != _order_readback_hash(readback):
                    return False
                if readback.get("status") not in {"pending", "confirmed", "cancelled"}:
                    return False
                if name == "confirm_order" and readback.get("status") != "confirmed":
                    return False
                if name == "abandon_pending_order" and (readback.get("status") != "cancelled" or not readback.get("abandoned")):
                    return False
                ...
                if name == "set_order_fulfillment":
                    ...
                    if not isinstance(details, Mapping):
                        return False
                    if arguments.get("fulfillment_at"):
                        try:
                            requested_at = datetime.fromisoformat(str(arguments["fulfillment_at"]))
                            stored_at = datetime.fromisoformat(str(details.get("fulfillment_at") or ""))
                            if requested_at.tzinfo is None or stored_at != requested_at:
                                return False
                        except ValueError:
                            return False
                ...
                canonical_items = [
                    {
                        ...
                        "required_modifier_groups": item.get("required_modifier_groups") or (),
                        "removable_ingredients": item.get("removable_ingredients") or (),
                ...
                facts.setdefault("canonical_items", []).append(
                    {
                        ...
                        "modifier_options": match.get("modifier_options") or (),
                        "required_modifier_groups": match.get("required_modifier_groups") or (),
                        "removable_ingredients": match.get("removable_ingredients") or (),
