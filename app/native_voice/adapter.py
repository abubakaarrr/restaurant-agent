...
class RealtimeConfig:
    ...
    instructions: str = (
        ...
        "For pickup or delivery, collect the customer name and callback phone before "
        "adding the first item. If either is missing, ask: What name and callback phone "
        "number should I use for this order? Retain all the caller's item choices, "
        "corrections, and notes while asking; do not make them repeat the order. "
        "Before adding an item, use check_menu_item_availability with only its base "
        "item name, without quantities, sides, or preparation requests, to resolve its canonical "
        "name and modifier option IDs. If that lookup is ambiguous, get_full_menu can "
        "resolve the base item and its options before asking the caller to repeat anything. "
        "Pass the canonical item name separately from "
...
class NativeVoiceAdapter:
    ...
    def _confirmation_sentence(outcome: ToolOutcome) -> str:
        ...
        if outcome.pending:
            ...
            if outcome.name == "update_confirmed_booking":
                booking_id = proposed.get("booking_id") or outcome.facts.get("booking_id") or ""
                labels = {
                    "date": "date", "time": "time", "party_size": "party size",
                    "preferred_location": "seating area", "customer_name": "name",
                    "seating_preference": "seating preference", "seating_backup": "backup seating",
                    "seating_avoid": "seating to avoid", "dietary": "dietary note",
                    "occasion": "occasion", "extra_notes": "guest note", "notes": "guest note",
                }
                details = []
                for key, label in labels.items():
                    if key not in outcome.arguments:
                        continue
                    value = proposed.get(key)
                    if value in (None, ""):
                        details.append(f"clear the {label}")
                    elif key == "party_size":
                        details.append(f"the party size to {int(value)} guests")
                    else:
                        details.append(f"{label} to {value}")
                if not details:
                    details = NativeVoiceAdapter._format_booking_update_fields(proposed)
                return (
                    f"For booking reference {booking_id}, I can change "
                    f"{', '.join(details)}. Should I make that change?"
                )
        ...
        elif outcome.name == "abandon_pending_order":
            booking_id = outcome.facts.get("booking_id") or (outcome.result or {}).get("booking_id")
            sentence = (
                "I have left the pre-order unplaced. Your table reservation remains confirmed."
                if booking_id else "I have left the unconfirmed order unplaced."
            )
    ...
    async def _finalize_caller_turn(self, transcript: str, *, generation: int) -> None:
        ...
        # A question or unclear utterance does not withdraw a proposal the
        # caller already heard. Only an explicit refusal/correction does.
        if self._native_service is not None and affirmation == "negative":
